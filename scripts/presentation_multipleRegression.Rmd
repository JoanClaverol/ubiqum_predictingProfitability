...
```{r train and test creation, warning=F, message=F, echo=T}
# train and test
...
## 2n pre-process: feature selection {.flexbox .vcenter}
...
## 3rd data exploration: stars analysis {.flexbox .vcenter}
...
ex_prod_dummy %>%
  ...
  gather(key = "num_stars", value = "value", x5, x4, x3, x2, x1) %>% 
  ggplot(aes(x = value, y = Vol, color = num_stars)) +
    geom_point() +
    ...
    theme(axis.text.y = element_text(),
          ...
          legend.title = element_blank()) +
    labs(title = "Total sales in relation of the star number", 
